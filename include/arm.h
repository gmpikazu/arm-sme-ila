...
class ArmSme {
  ...
  ExprRef _GetElement(const ExprRef &addr,
  ...
  // TODO tile_idx MUST BE in range [0, SVL_B-1] during instr decode: WE ACHIEVE
  // THIS BY RESTRICTING THE BIT WIDTH OF THE tile_idx INPUT
  ExprRef _GetTypedHorizontalSlice(
      const ExprRef &row_idx, const ExprRef &tile_idx,
      const NumericType &element_size_bits); // topmost row is index 0
  ExprRef _GetTypedVerticalSlice(
      const ExprRef &col_idx, const ExprRef &tile_idx,
      const NumericType &element_size_bits); // rightmost col is index 0

  // @param[in] is_vertical takes BoolConst(true) or BoolConst(false)
  ExprRef GetTypedSlice(const ExprRef &is_vertical, const ExprRef &tile_idx,
                        const ExprRef &slice_idx,
                        const NumericType &element_size_bits);
  // TODO In SME instructions the tile slice is selected by the sum of a 32-bit
  // general-purpose register (slice index register Ws) and an immediate, modulo
  // the number of slices in the named tile.

  // @brief extracts element from slice [MSB...LSB], rightmost element is index
  // 0
  ExprRef ExtractElement(const ExprRef &vector, const NumericType &idx,
                         const NumericType &element_size_bits);
