...
namespace arm {
    ...
    ExprRef ArmSme::_GetElement(const ExprRef& addr, const NumericType& element_size_bits) {
        switch (element_size_bits) {
            ...
            default: throw std::runtime_error("GetElement(): invalid element size_bits");
        }
    }
    ...
    ExprRef ArmSme::_GetTypedVerticalSlice(const ExprRef& col_idx, const ExprRef& tile_idx, const NumericType& element_size_bits) {
        ...
        ExprRef slice = _GetElement(_ToMemoryAddress(tile_idx, col), element_size_bits); // first row
        ...
        for (size_t i = 1; i < dim; i++) { // so loops all rows of this tile
            ...
            slice = Concat(_GetElement(_ToMemoryAddress(tile_idx + BvConst(i * num_tiles, ZA_ADDR_WIDTH), col), element_size_bits), slice);
        }
        return slice;
    }
    ...
    ExprRef ArmSme::ExtractElement(const ExprRef& vector, const NumericType& idx, const NumericType& element_size_bits) {
    ...
    void ArmSme::AddInstructions() {
        { // BUG DUMMY INSTR
            ...
            instr.SetUpdate(z_regs[0], ExtractElement(GetTypedSlice(BoolConst(true), BvConst(0, ZA_ADDR_WIDTH), BvConst(0, ZA_ADDR_WIDTH), BYTE), 0, BYTE));
        }
